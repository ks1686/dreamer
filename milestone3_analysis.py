...
random_means = [
    random_bl.get(t, {}).get("return_mean", float("nan")) for t in COMPARE_TASKS
]
d4pg_vals = [baselines.get(t, {}).get("d4pg_100m", float("nan")) for t in COMPARE_TASKS]
task_labels = [pretty(t) for t in COMPARE_TASKS]

x = np.arange(len(COMPARE_TASKS))
w = 0.25
fig1, ax1 = plt.subplots(figsize=(10, 4.5))
ax1.bar(
    x - w, random_means, w, label="Random Policy (ours)", color="#888888", alpha=0.8
)
ax1.bar(
    x, dreamer_means, w, label="Dreamer 1M steps (paper)", color="#377eb8", alpha=0.85
)
ax1.bar(
    x + w, d4pg_vals, w, label="D4PG 100M steps (paper)", color="#e41a1c", alpha=0.75
)
ax1.set_xticks(x)
ax1.set_xticklabels(task_labels, rotation=30, ha="right", fontsize=8)
ax1.set_ylabel("Episode Return", fontsize=9)
ax1.set_title(
    "Three-Way Comparison: Random Policy (ours) vs Dreamer (1M) vs D4PG (100M)",
    fontsize=10,
    fontweight="bold",
)
...
for ax, (task_key, title) in zip(
    ...
    ax.bar(
        scale_strs, means, yerr=stds, capsize=4, color="#4daf4a", alpha=0.8, width=0.5
    )
    dmc_key = f"dmc_{task_key}"
    dr_mean = final_scores(dmc_key).mean()
    d4pg_v = baselines.get(dmc_key, {}).get("d4pg_100m", None)
    ax.axhline(
        dr_mean, color="#377eb8", ls="--", lw=1.5, label=f"Dreamer 1M ({dr_mean:.0f})"
    )
    if d4pg_v:
        ax.axhline(
            d4pg_v, color="#e41a1c", ls=":", lw=1.5, label=f"D4PG 100M ({d4pg_v:.0f})"
        )
...
fig2.suptitle(
    "Action-Scale Failure Mode: Episode Return vs. Control Authority\n"
...
ax3a.set_title(
    "Object Interaction Tasks vs. DMC Tasks\n(Random Policy Baseline, ours)",
    fontsize=9,
    fontweight="bold",
)
ax3a.legend(
    [patch_gray, patch_purple], ["Paper tasks", "Manipulator (new)"], fontsize=7
)
ax3a.grid(axis="y", alpha=0.3)

energy_vals = [random_bl.get(t, {}).get("energy_mean", 0) / 1000 for t in ref_tasks] + [
...
ax3b.set_title(
    "Energy Metric: Action Effort per Step\n(unreported in original paper)",
    fontsize=9,
    fontweight="bold",
)
ax3b.legend(
...
ax4.barh(
    ...
    capsize=3,
    height=0.55,
    label="Dreamer 1M (mean ± 95% CI, bootstrap, 5 seeds)",
)
...
ax4.scatter(
    ...
    marker="D",
    s=40,
    label="D4PG 100M steps (reported)",
)
...
ax4.set_title(
    "Bootstrap 95% CI on Final Performance: Dreamer vs. D4PG",
    fontsize=10,
    fontweight="bold",
)
...
for k in manip_keys:
    d = manip.get(k, {})
    print(
        f"  {k}: return={d.get('return_mean', 0):.2f}  energy/step={d.get('energy_per_step_mean', 0):.2f}"
    )
