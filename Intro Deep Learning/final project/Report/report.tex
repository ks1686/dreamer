...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    Config 2: FC-Dropout + CONV-noBN  & $99.13\% \pm 0.05\%$          & $99.19\% \pm 0.05\%$          & $98.91\%$ & 93.8 s \\
    Config 3: FC-noDropout + CONV-BN  & $98.96\% \pm 0.12\%$          & $99.14\% \pm 0.06\%$          & $99.75\%$ & 101.3 s \\
...
Dropout (holding BN constant), Dropout contributes a $\approx$0.18--0.20\% gain in test
accuracy. The corresponding gain from BN alone (holding Dropout constant) is
$\approx$0.02--0.18\%.
...
\textbf{BN improves stability.} Config 3 (no Dropout, with BN) shows the highest
standard deviation (0.12\%), compared to $\approx$0.05--0.08\% for all other configs.
This suggests that without Dropout, BN alone is insufficient to fully stabilize training.
The addition of Dropout in Config 1 brings the std down to 0.05\%, the lowest of all runs.
...
\begin{enumerate}
  ...
  \item BN provides a secondary benefit in accuracy and noticeably improves training
        stability (lower variance across seeds) when combined with Dropout.
...
combining Dropout and Batch Normalization is a sound default strategy for convolutional
networks.
